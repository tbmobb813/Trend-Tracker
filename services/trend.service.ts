...
// Simple API shim for trends. Replace with a real HTTP call later.
...
export async function fetchTrends(): Promise<Trend[]> {
  // Try real API if configured, otherwise fall back to local mock data.
  if (typeof process !== 'undefined' && process.env && process.env.EXPO_PUBLIC_API_URL) {
    try {
      return await api.get<Trend[]>('/trends');
    } catch (err) {
      // eslint-disable-next-line no-console
      console.warn('API fetch failed, falling back to mock trends', err);
    }
  }

  // Simulate network latency (kept small for dev)
